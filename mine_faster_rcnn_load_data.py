...
with tf.Session() as sess:
    ...
    if(path.is_file()):
        ...
        for i in range(1):
            saver.restore(sess, "/home/hanz/Documents/2018/weights/tmp/model.ckpt")
...
#                                                      feed_dict={im_placeholder:np.expand_dims(img, axis=0), y_:np.expand_dims(y_hat, axis=0), y_reg: np.expand_dims(encoded_training, axis=0)})
            mult_net1_s, mult_net2_s, bm_y_s, bm_net1_s, argmax_1s, argmax_2s, _, net_cnn_s, net2_s, net1_s = sess.run([mult_net11, mult_net2, bm_y_, bm_net1, argmax_1, argmax_2, train_step, net_cnn, net2, net1],
        ...
        g2 = np.greater(mult_net1_s[:,:,0], np.array([0.2]))
        dec = []
        c_dec = []
